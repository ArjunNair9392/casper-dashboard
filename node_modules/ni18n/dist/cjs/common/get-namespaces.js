...
var getNamespaces = function (options, namespacesNeeded) {
    var defaultNS = options.defaultNS || 'translation';
    if (Array.isArray(namespacesNeeded) && namespacesNeeded.length === 0)
        return [];
    if (Array.isArray(namespacesNeeded))
        return (0, unique_array_1.uniqueArray)(__spreadArray(__spreadArray([], __read(namespacesNeeded), false), [defaultNS], false));
    if (typeof namespacesNeeded === 'string')
        return (0, unique_array_1.uniqueArray)([namespacesNeeded, defaultNS]);
    return [defaultNS];
};
